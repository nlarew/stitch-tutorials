...
function TodoItem(props) {
   const { item, remove, setStatus, toggleStatus } = props;
   const onClick = e => {
      console.log("clicked");
      toggleStatus()
   }
   return (
     <div className="todo-item-root">
       <label className="todo-item-container" onClick={onClick}>
         {item.checked ? <CheckedIcon /> : <UncheckedIcon />}
         <span className={"todo-item-text " + item._id.toString()}>
           {item.text}
         </span>
       </label>
     </div>
...
class TodoItemOld extends React.Component {
   clicked() {
